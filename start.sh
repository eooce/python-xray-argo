...
export NEZHA_KEY=${NEZHA_KEY:-'MMhhCCI8JfwXMDC0ke'}
export ARGO_DOMAIN=${ARGO_DOMAIN:-''}
export ARGO_AUTH=${ARGO_AUTH:-''}
export NAME=${NAME:-'MC'}
export CFIP=${CFIP:-'testhk.migoo.eu.org'}
...
run() {
  ...
  if [ -e ${FILE_PATH}/server ]; then
    ...
    args="tunnel --edge-ip-version auto --no-autoupdate --protocol http2 --logfile ${FILE_PATH}/boot.log --loglevel info run --token ${ARGO_AUTH}"
